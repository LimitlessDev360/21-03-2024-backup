...
class VendorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $restaurant->logo = Helpers::upload( dir: 'restaurant/', format: 'png',  image: $request->file('logo')) ??null;
        $restaurant->cover_photo = Helpers::upload( dir: 'restaurant/cover/',  format:'png', image:  $request->file('cover_photo'))??null;
        $restaurant->address = $request->address;
        $restaurant->latitude = $request->latitude;
        $restaurant->longitude = $request->longitude;
        $restaurant->vendor_id = $vendor->id;
        $restaurant->zone_id = $request->zone_id;
        $restaurant->tax = $request->tax ?? 0.00;
        $restaurant->gst_no = $request->gst_no ??null;
        $restaurant->tax_include = $request->tax_includev??null;
        $restaurant->halal_no = $request->halal_no??null;
        $restaurant->halal_certificate = Helpers::upload( dir: 'restaurant/halal/',  format:'png', image:  $request->file('halal_certificate')) ?? null;
    ...
    public function view($restaurant,Request $request, $tab = null, $sub_tab = 'cash')
    {
        ...
        return view('admin-views.vendor.pending_list_view', compact('restaurant'));
    }
    ...
    public function restaurant_status(Restaurant $restaurant, Request $request)
    {
        ...
        if($request->menu == 'instant_order'  || $request->menu == 'customer_date_order_sratus' ){
