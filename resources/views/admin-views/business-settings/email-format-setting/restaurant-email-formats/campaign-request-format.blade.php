    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','campaign_request_mail_status_restaurant')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                ...
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_Restaurant_requests_to_join_a_campaign,_they_will_receive_an_automated_mail_for_successful_registration.')}}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-text-on="<p>{{translate('If_enabled,_restaurants_will_receive_an_automated_confirmation_mail_that_their_join_request_is_successful')}}</p>"
                                data-text-off="<p>{{translate('If_disabled,_restaurants_will_not_receive_any_confirmation_mail_on_campaign_join_requests.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['restaurant','campaign-request',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','restaurant')->where('email_type', 'campaign_request')->first())
                 @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.email-setup', ['restaurant','campaign-request']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        ...
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','restaurant')->where('email_type', 'campaign_request')->first())
