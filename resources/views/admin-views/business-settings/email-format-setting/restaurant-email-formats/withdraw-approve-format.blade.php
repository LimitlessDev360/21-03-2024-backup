    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','withdraw_approve_mail_status_restaurant')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_on_Withdraw_Approval')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_vendor_requests_a_withdrawal_and_Admin_approves_it,_the_vendor_will_get_an_automated_Withdraw_Approval_email_from_the_system.')}}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-text-on="<p>{{translate('If_enabled,_vendors_will_receive_an_approval_mail_for_requesting_a_withdrawal.')}}</p>"
                                data-text-off="<p>{{translate('If_disabled,_vendors_will_not_receive_any_Withdraw_Approval_mail.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['restaurant','withdraw-approve',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','restaurant')->where('email_type', 'withdraw_approve')->first())
                @php($template= $template ?? $data?->email_template ?? 6)
                <form action="{{ route('admin.business-settings.email-setup', ['restaurant','withdraw-approve']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','restaurant')->where('email_type', 'withdraw_approve')->first())
