    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','campaign_deny_mail_status_restaurant')->first())
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.business-settings.email-status',['restaurant','campaign-deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','restaurant')->where('email_type', 'campaign_deny')->first())
                @php($template=$template?$template:($data?$data->email_template:7))
                <form action="{{ route('admin.business-settings.email-setup', ['restaurant','campaign-deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','restaurant')->where('email_type', 'campaign_deny')->first())
