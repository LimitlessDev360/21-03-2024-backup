    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','campaign_approve_mail_status_restaurant')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_on_Campaign_Join_Approval')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_restaurant_requests_to_join_a_Campaign_and_Admin_approves_their_joining_request,_they_will_get_an_automated_Approval_email_from_the_system.')}}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-text-on="<p>{{translate('If_enabled,_restaurants_will_receive_an_email_when_Admin_approves_their_Campaign_Join_Request.')}}</p>"
                                data-text-off="<p>{{translate('If_disabled,_restaurants_will_not_receive_an_email_when_Admin_approves_their_Campaign_Join_Request.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['restaurant','campaign-approve',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','restaurant')->where('email_type', 'campaign_approve')->first())
                 @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.email-setup', ['restaurant','campaign-approve']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','restaurant')->where('email_type', 'campaign_approve')->first())
