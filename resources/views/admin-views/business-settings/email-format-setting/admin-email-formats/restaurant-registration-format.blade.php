    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','restaurant_registration_mail_status_admin')->first())
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.business-settings.email-status',['admin','restaurant-registration',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','admin')->where('email_type', 'restaurant_registration')->first())
                 @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.email-setup', ['admin','restaurant-registration']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','admin')->where('email_type', 'restaurant_registration')->first())
